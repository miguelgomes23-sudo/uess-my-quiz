...
export default function PerfilPage() {
  ...
  const [currentالUserUsername, setCurrentUserUsername] = useState<string | null>(null);
  ...
  if (loading) {
    return <div className="flex min-h-dvh items-center justify-center">A carregar perfil...</div>;
  }

  if (!perfilData) {
    return <div className="flex min-h-dvh items-center justify-center">Este perfil não existe ou o username está errado.</div>;
  }

  const isMyProfile = currentUserUid === perfilData.uid;

  return (
    <div className="flex min-h-dvh flex-col pb-20 sm:mx-auto sm:max-w-lg sm:border-x sm:border-gray-100 bg-white">
      <header className="sticky top-0 z-10 flex items-center gap-6 bg-white/90 px-6 py-4 backdrop-blur-md">
      ...
      <div className="relative h-32 bg-[#e6dfff]" />
      
      <div className="px-6 pb-6">
        <div className="relative flex justify-between items-end -mt-12 mb-4">
          <div className="size-24 rounded-full border-4 border-white bg-gray-200 overflow-hidden flex items-center justify-center shadow-sm">
          ...
          {isMyProfile ? (
            <Link href="/editar-perfil" className="rounded-full border border-gray-200 bg-white px-4 py-1.5 text-sm font-bold text-foreground hover:bg-gray-50 shadow-sm transition-colors active:scale-95">
              Editar Perfil
            </Link>
          ) : (
            <button 
              onClick={handleToggleSeguir}
              className={`rounded-full px-6 py-1.5 text-sm font-bold transition-colors shadow-sm active:scale-95 ${
                isFollowing 
        ...
        <p className="mt-3 text-sm text-foreground">
          Adoro quizzes e testar a paciência dos meus amigos. 🕹️
        </p>
        ...
        {isMyProfile && (
          <Link href="/criar/modo" className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-accent px-6 py-3 font-semibold text-white shadow-sm transition-colors hover:bg-accent-hover active:scale-[0.98]">
      ...
      {isMyProfile ? (
        <div className="border-b border-gray-100 flex px-6">
          <div onClick={() => setTabAtiva("quizzes")} className={`flex-1 py-4 text-center cursor-pointer transition-colors ${tabAtiva === "quizzes" ? "border-b-2 border-accent font-bold text-foreground" : "text-muted font-medium hover:bg-gray-50"}`}>
            Os Meus Quizzes
          </div>
          <div onClick={() => setTabAtiva("resultados")} className={`flex-1 py-4 text-center cursor-pointer transition-colors ${tabAtiva === "resultados" ? "border-b-2 border-accent font-bold text-foreground" : "text-muted font-medium hover:bg-gray-50"}`}>
            Resultados
          </div>
        </div>
      ) : (
        <div className="border-b border-gray-100 flex px-6">
          <div className="flex-1 py-4 text-center border-b-2 border-accent font-bold text-foreground">
            Quizzes Criados
          </div>
        </div>
      )}
      ...
      {/* CONTEÚDO DAS TABS */}
      <div className="flex flex-col bg-gray-50/50 min-h-[300px]">
          ...
          <div className="flex flex-col">
            {userQuizzes.length === 0 ? (
              ...
              userQuizzes.map((quiz) => (
                <article key={quiz.id} className="border-b border-gray-100 p-6 flex flex-col gap-3 bg-white">
                  ...
                  <div className="flex gap-3 mt-2">
                    {!isMyProfile ? (
                      <Link href={`/jogar/${quiz.id}`} className="flex-1 bg-accent text-white text-center py-2.5 rounded-xl text-sm font-semibold hover:bg-accent-hover transition-colors shadow-sm active:scale-95">
                        Jogar Quiz
                      </Link>
                    ) : (
                      <button onClick={() => copiarLink(quiz.id)} className="flex-1 flex items-center justify-center gap-2 bg-gray-100 text-foreground text-center py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-200 transition-colors shadow-sm active:scale-95">
