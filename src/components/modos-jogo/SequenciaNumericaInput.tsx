...
export default function SequenciaNumericaInput({ index, onUpdate }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-sm font-medium text-muted">Que número vem a seguir? Dá 4 opções.</p>
      <div className="grid grid-cols-2 gap-2 w-full">
        {options.map((opt, i) => (
          <label key={i} className={`flex items-center gap-3 rounded-xl border p-2 transition-colors ${correctAnswer === i ? "border-accent bg-accent/10" : "border-white/10 bg-surface"}`}>
            ...
            <input type="number" value={opt} onChange={(e) => setOptions(prev => prev.map((o, idx) => idx === i ? e.target.value : o))} placeholder={`Hipótese ${i + 1}`} className={`${fieldInputClass} border-0 bg-transparent px-1 py-1 text-sm focus:ring-0`} />
          </label>
        ))}
      </div>
    </div>
