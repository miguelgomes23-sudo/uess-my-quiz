...
export default function OQueAconteceInput({ index, onUpdate }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="fixed inset-0 z-[100] flex flex-col bg-black/95 animate-in fade-in zoom-in-95 duration-200 p-4 justify-center">
          <div className="flex flex-col gap-5 bg-white p-6 rounded-3xl w-full max-w-sm mx-auto shadow-2xl">
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-foreground mt-2">Ajustar Clip</h3>
              <p className="text-xs text-muted">A janela tem exatos <b>{MAX_DURATION}s</b>.</p>
            </div>
            
            {/* O modal mantém um tamanho aceitável para caber no ecrã */}
      ...
      {/* ÁREA PRINCIPAL DA PÁGINA - Agora a 100% de largura (w-full)! */}
      {!videoUrl ? (
      ...
      <div className="grid grid-cols-2 gap-2 w-full mt-2">
        {options.map((opt, i) => (
          <label key={i} className={`flex flex-col gap-2 rounded-xl border p-2 transition-colors ${correctAnswer === i ? "border-accent bg-accent/10" : "border-white/10 bg-surface"}`}>
            ...
            <textarea value={opt} onChange={(e) => setOptions(prev => prev.map((o, idx) => idx === i ? e.target.value : o))} placeholder="O que vai acontecer?" rows={2} className={`${fieldInputClass} resize-none border-0 bg-transparent px-1 py-0 text-sm focus:ring-0 leading-tight`} />
          </label>
        ))}
      </div>
    </div>
