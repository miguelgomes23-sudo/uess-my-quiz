  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="fixed inset-0 z-[100] flex flex-col bg-black/95 animate-in fade-in zoom-in-95 duration-200 p-4 justify-center">
          <div className="flex flex-col gap-5 bg-white p-6 rounded-3xl w-full max-w-sm mx-auto shadow-2xl">
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-foreground mt-2">O Dilema</h3>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleConfirmCrop} disabled={isUploading} className="flex-1 rounded-2xl bg-accent py-3 text-sm font-bold text-white hover:bg-accent-hover disabled:opacity-50">{isUploading ? "A Guardar..." : "Cortar e Guardar"}</button>
            </div>
          </div>
        </div>
      )}

      {/* ÁREA PRINCIPAL DA PÁGINA (w-full) */}
      {!videoUrl ? (
      ...
      <div className="flex flex-col gap-3 w-full mt-2">
        {options.map((opt, i) => (
          <label key={i} className={`flex flex-col gap-2 rounded-xl border p-3 transition-colors ${correctAnswer === i ? "border-accent bg-accent/10" : "border-white/10 bg-surface"}`}>
            <div className="flex items-center gap-2">
              <input type="radio" checked={correctAnswer === i} onChange={() => setCorrectAnswer(i)} className="size-4 accent-accent" />
              <span className="text-xs font-bold text-muted uppercase">Caminho {i + 1}</span>
            </div>
            <textarea value={opt} onChange={(e) => setOptions(prev => prev.map((o, idx) => idx === i ? e.target.value : o))} placeholder="Escreve uma decisão ou desfecho possível..." rows={2} className={`${fieldInputClass} resize-none border-0 bg-transparent px-1 py-0 text-sm focus:ring-0 leading-tight`} />
          </label>
        ))}
      </div>
    </div>
