...
export default function ContinuaLetraInput({ index, onUpdate }: Props) {
  ...
  const [palavra, setPalavra] = useState(""); // Em vez de array de opções, apenas 1 palavra
  ...
  useEffect(() => {
    const isValid = audioUrl !== null && palavra.trim() !== "";
    onUpdate(isValid, { audioUrl, respostaCorreta: palavra.toUpperCase() });
  }, [audioUrl, palavra, onUpdate]);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.size > 20 * 1024 * 1024) return alert("O áudio é demasiado pesado.");
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="fixed inset-0 z-[100] flex flex-col bg-black/90 animate-in fade-in zoom-in-95 duration-200 p-4 justify-center">
          <div className="flex flex-col gap-6 bg-white p-6 rounded-3xl w-full max-w-sm mx-auto shadow-2xl">
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-foreground mt-2">Escolher o Excerto</h3>
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={togglePreviewPlay} className={`mt-2 rounded-xl py-3 text-sm font-bold transition-colors ${isPlaying ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-gray-100 text-foreground hover:bg-gray-200'}`}>
                {isPlaying ? "⏹ Parar Som" : "▶️ Ouvir Excerto"}
              </button>
            </div>
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-gray-200 bg-surface p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
              <span className="text-sm font-bold text-foreground">Excerto Final</span>
      ...
      <div className="flex flex-col gap-2 mt-2">
        <span className="text-sm font-medium text-muted">A Palavra Secreta a Completar</span>
        <input 
          type="text" 
          value={palavra} 
          onChange={(e) => setPalavra(e.target.value.toUpperCase())} 
          placeholder="Ex: AMOR" 
          className={`${fieldInputClass} uppercase font-bold`} 
        />
      </div>
    </div>
  );
